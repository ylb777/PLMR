...
def parse_args():
    ...
    parser.add_argument('--sparsity_percentage', type=float, default=0.2, help='每一层的学习到的百分比')
    parser.add_argument('--dim_reduction_start', type=int, default=3, help='从这一层开始令牌递减')
    parser.add_argument('--dim_reduction_end', type=int, default=7, help='从这一层结束令牌递减')
    parser.add_argument('--max_length', type=int, default=256, help='Max sequence length [default: 256]')
    parser.add_argument('--data_dir', type=str, default='/newdisk/ylb/data/beer', help='Path of the dataset')
    parser.add_argument('--data_type', type=str, default='beer', help='0:beer,1:hotel')
    parser.add_argument('--aspect', type=int, default=0, help='The aspect number of beer review [0, 1, 2]')
    parser.add_argument('--annotation_path', type=str, default='/newdisk/ylb/data/beer/annotations.json', help='Path to the annotation')
    parser.add_argument('--batch_size', type=int, default=64, help='Batch size [default: 100]')
    parser.add_argument('--balance', type=bool, default=True, help='Batch size [default: 100]')
    parser.add_argument('--epochs', type=int, default=12, help='Batch size [default: 100]')
...
for epoch in range(args.epochs):
    ...
    print("Annotation")
    precision, recall, f1_score, accuracy, sparsity, micro_precision, micro_recall, micro_f1 = annotation_plmr(model, annotation_loader, device, args)
    print("precision:{:.4f} recall:{:.4f} accuracy:{:.4f} f1-score:{:.4f}".format(precision, recall, accuracy, f1_score))
    print("the Annotation result sparsity :{:.4f}  micro_precision :{:.4f}  micro_recall :{:.4f}  micro_f1 :{:.4f} ".format(sparsity, micro_precision, micro_recall, micro_f1))
