...
def annotation_plmr(model, dataset, device, args):
    ...
    for batch_index, data in enumerate(dataset):
        ...
        # Compute micro statistics
        true_pos_count, predicted_pos_count, real_pos_count = compute_micro_stats(annotations, rationales)
    ...
    micro_precision = num_true_positives / num_predicted_positives
    micro_recall = num_true_positives / num_real_positives
    micro_f1 = 2 * (micro_precision * micro_recall) / (micro_precision + micro_recall)
    sparsity = num_predicted_positives / total_words

    return precision, recall, f1_score, accuracy, sparsity, micro_precision, micro_recall, micro_f1


def compute_micro_stats(true_labels, predicted_labels):
